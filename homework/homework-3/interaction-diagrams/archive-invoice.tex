...
\draw (system) -- (DbSupport) node[midway]{$\shortdownarrow$ 3, 5};
...
\begin{enumerate}
  \item \texttt{b:=getInvoiceByID(id:String):boolean}
  \item \texttt{b:=getInvoiceByID(id:String):boolean}
  \item \texttt{i:=getInvoice(id:String):invoice}
  \item \texttt{b:=archive():boolean}
  \item \texttt{b:=putInvoice(i):boolean}
\end{enumerate}
\end{center}
