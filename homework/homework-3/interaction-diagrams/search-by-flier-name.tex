...
\title{Interaction Diagram - Search by Flight Missed}
\author{ Nick Riesen }
...
\begin{enumerate}
  \item 1:L:=searchByTimeDate(time:TimeDate):List$\langle$invoice$\rangle$
  \item 2:L:=searchByTimeDate(time:TimeDate):List$\langle$invoice$\rangle$
  \item 3:L:=searchByTimeDate(time:TimeDate):List$\langle$invoice$\rangle$
