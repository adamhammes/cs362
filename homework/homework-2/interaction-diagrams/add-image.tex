...
\title{Interaction Diagram - Add Image}
\author{ Nick Riesen }
...
\draw (system) -- (db) node[midway]{$\shortleftarrow$ 4 9};
\draw (system) -- (photographer) node[midway]{$\shortrightarrow$ 5};
\draw (photographer) -- (image) node[midway]{$\shortdownarrow$ 6 7 8};
...
\begin{enumerate}
  \item b:=addImage(img:ImageFile, caption:string):boolean
  \item b:=addImage(img:ImageFile, caption:string):boolean
  \item au:=getActiveUserId():string
  \item p:=getPhotographer(au:string):photographer
  \item b:=addImage(img:ImageFile, caption:string):boolean
  \item create()
  \item b:=setImage(img: ImageFile):boolean
  \item b:=setCaption(caption:string):b
