...
\title{Interaction Diagram - List All Versions of a Book}
...
\begin{enumerate}
  \item \textttt{v:= listAllVersions(bid:String, username:String):List$\langle$Version$\rangle$ }
  \item \textttt{v:= listAllVersions(bid:String, username:String):List$\langle$Version$\rangle$}
  \item \textttt{book:= getBook(bid:String):Book}
  \item \textttt{v:= getVersions():List\langle$Version$\rangle$}
\end{enumerate}
\end{center}
